...
def categoriesJSON():
    ...
    return jsonify(CatalogItem = [k.serializeCatalogItem for k in (items)])
...
def showCategories():
    ...
    last_added = session.query(CatalogItem.name,Category.name,CatalogItem.id,Category.id).select_from(join(CatalogItem,Category)).order_by(desc(CatalogItem.id)).limit(10).all()
    if not categories:
        flash('No Category exists. Create New')
    if 'username' not in login_session:
        return render_template('publiccatalog.html', categories=categories,last_added=last_added)
    else:
        return render_template('catalogs.html', categories=categories,last_added=last_added)
...
def showCategory(category_name):
    ...
    itemcount = session.query(CatalogItem).filter_by(
        category_id=category.id).count()
   
    if 'username' not in login_session or creator.id != login_session['user_id']:
        return render_template('publiccateoryitems.html', items=items, category=category, creator=creator)
    else:
        return render_template('categoryitem.html', items=items, category=category, creator=creator,itemcount=itemcount)
...
def editCategoryItem(category_name,item_name):
    ...
    if login_session['user_id'] != editedItem.user_id:
        return "<script>function myFunction() {alxert('You are not authorized to edit menu items to this restaurant. Please create your own restaurant in order to edit items.');}</script><body onload='myFunction()'>"
    if request.method == 'POST':
        if request.form['name']:
            editedItem.name = request.form['name']
        if request.form['description']:
            editedItem.description = request.form['category']
        catid = session.query(Category.id).filter_by(name= request.form['category']).one()       
        editedItem.category_id = catid[0]
        session.add(editedItem)
        session.commit()
        flash('Category Item Successfully Edited %s' % catid )
        return redirect(url_for('showCategories'))
    else:
